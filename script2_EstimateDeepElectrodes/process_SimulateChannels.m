function varargout = process_SimulateChannels( varargin )
...
function sProcess = GetDescription()
  ...
  sProcess.Comment     = 'Depth Electrode from Surface Electrode';
  ...
  %
  sProcess.options.ScoutRadius.Comment = 'Scout Radius around Deep Electrode, in mm: ';
  ...
  %
  sProcess.options.magn.Comment   = 'Compute magnitude [False is experimental] ';
...
% recordings that could be obtained from depth electrodes. 
%
% For each intended depth electrode, a scout is created with the dipoles
% located within some given distance. These dipoles were averaged over 
% each canonical direction, and then the magnitude is extracted.
%
...
if constr
  ...
  % quick trick, I'm making it a tensor for ease of computation
  idx = 0:(meta.DE-1);
  ...
  if magn
    ...
    for ii = 1:meta.DE
      ChanVec = reshape( tensChans(ii,:,:), meta.TT, 3);
      %disp('Size of matrix')
      %disp(size(ChanVec))
